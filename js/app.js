...
////////////////////////////////////////////////////////////
var showTopAnswerers = function(answer) {
        console.log(answer);
        var result = $('.templates .answer').clone();

        var answerElem = result.find('.TopAnswer a');
        answerElem.attr('href', answer.user.link);
        answerElem.text(answer.user.display_name);
       
        var answerElem2 = result.find('.reputation');
        answerElem2.text(answer.user.reputation);

         var answerElem3 = result.find('.image img');
        answerElem3.attr('src', answer.user.profile_image);
...
var getTopAnswerers = function(tags) {
    ...
    $.ajax({
        ...
        .done(function(result) { //this waits for the ajax to return with a succesful promise object
            ...
            $('.search-results').html(searchResults);
            //$.each is a higher order function. It takes an array and a function as an argument.
            //The function is executed once for each item in the array.{tag}/
            $.each(result.items, function(i, item) {
